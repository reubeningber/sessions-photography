...
The full site is live at `/`. It previously ran in a temporary "coming soon"
mode (an email-capture page at `/`, with the real site staged at `/preview`
behind Cloudflare Access) ahead of launch — see git history before the
`/preview` promotion commit if that setup is ever needed for reference.
Booking inquiries from the contact form land under **Forms →
booking-inquiry** in the Netlify dashboard.
